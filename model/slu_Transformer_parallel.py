...
class TagTransformerParallel(nn.Module):

    def __init__(self, config):
        super(TagTransformerParallel, self).__init__()
        self.config = config
        self.cell = config.encoder_cell
        self.word_embed = nn.Embedding(config.vocab_size, config.embed_size, padding_idx=0)
        self.rnn = getattr(nn, self.cell)(config.embed_size, config.hidden_size // 2, num_layers=config.num_layer_rnn, bidirectional=True, batch_first=True)
