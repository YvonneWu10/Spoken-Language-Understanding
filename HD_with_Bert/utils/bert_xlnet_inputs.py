...
def prepare_inputs_for_bert_xlnet(sentences, word_lengths, tokenizer,
        cls_token_at_end=False, pad_on_left=False, cls_token='[CLS]', sep_token='[SEP]',
        pad_token=0, sequence_a_segment_id=0, cls_token_segment_id=0, pad_token_segment_id=0, device=None):
    ...
    for i in range(batch_size):
        ...
        for w in ws:
            if cls_token_at_end:
                ...
                # the index should +1 because [CLS] is at the beginning
                selected_index.append(len(ts) + 1)
            tok_w = tokenizer.tokenize(w)
            ts += tok_w

        # [SEP] token should be at the end   
        ts += [sep_token]

        # this is the token type ids, and is default to be all 0 in Bert
        si = [sequence_a_segment_id] * len(ts)
        if cls_token_at_end:
            ts = ts + [cls_token]
            si = si + [cls_token_segment_id]
        else:
            # for BERT, [CLS] at the beginning
            ts = [cls_token] + ts
    ...
    # add padding, pad all to the longest token
    ...
    if pad_on_left:
        ...
        # want it to be padded on right
        # the input mask to show valid words
        ...
        # index each word without overlap
    ...
    return {'tokens': tokens_tensor, 'token_lens': token_lens,
...
def prepare_inputs_for_bert_xlnet_act(act_inputs, word_vocab, label_vocab, tokenizer, device):
    '''
    inputs:
    ...
    for i in range(len(act_inputs)):
        ...
        # act_inputs
        if ai is not None:
            ai_words = [label_vocab.idx2act[x[0]] for x in ai.tolist()]  # [str] * #acts
            tok_ai = [
...
def prepare_inputs_for_bert_xlnet_slot_enums(slot_inputs, word_vocab, label_vocab, tokenizer, device):
    '''
    inputs:
    ...
    for i in range(len(slot_inputs)):
        ...
        # slot_inputs
        if si is not None:
            si_words = [label_vocab.idx2slot[x[0]] for x in si.tolist()]  # [str] * #slots
            tok_si = [
                tokenizer.tokenize(x)
                for x in si_words
            ]
...
def prepare_inputs_for_bert_xlnet_slot(act_slot_pairs, word_vocab, label_vocab, tokenizer, device):
    '''
    inputs:
    ...
    for i in range(len(act_slot_pairs)):
        ...
        if asp is not None:
            ...
            for pair in asp.tolist():
                # act
                pr_act_word = label_vocab.idx2act[pair[0]]
                pr_act_tokens = tokenizer.tokenize(pr_act_word)
                pr_act_token_ids = torch.LongTensor(
                    tokenizer.convert_tokens_to_ids(pr_act_tokens)).to(device)
                # slot
                pr_slot_word = label_vocab.idx2slot[pair[1]]
                pr_slot_tokens = tokenizer.tokenize(pr_slot_word)
...
def prepare_inputs_for_bert_xlnet_value(value_inps, value_outs, labels, word_vocab, label_vocab, tokenizer,
        cls_token_at_end=False, pad_on_left=False, cls_token='[CLS]', sep_token='[SEP]',
        pad_token=0, sequence_a_segment_id=0, sequence_b_segment_id=1,
        cls_token_segment_id=1, pad_token_segment_id=0, device=None):
    '''
...
def prepare_inputs_for_bert_xlnet_act_slot_value(act_inputs, act_slot_pairs, value_inps, value_outs, labels, word_vocab, label_vocab, tokenizer, cls_token_at_end=False, pad_on_left=False, cls_token='[CLS]', sep_token='[SEP]', pad_token=0, sequence_a_segment_id=0, sequence_b_segment_id=1, cls_token_segment_id=1, pad_token_segment_id=0, device=None):
    '''
    inputs:
        - act_inputs: list of {tensor(#acts, 1), None}
        - act_slot_pairs: list of {tensor(#value_inps, 2), None}
    ...
    act_inputs_token_ids = prepare_inputs_for_bert_xlnet_act(act_inputs, word_vocab, label_vocab, tokenizer, device)
    act_slot_pairs_token_ids = prepare_inputs_for_bert_xlnet_slot(act_slot_pairs, word_vocab, label_vocab, tokenizer, device)
    results = prepare_inputs_for_bert_xlnet_value(value_inps, value_outs, labels, word_vocab, label_vocab, tokenizer,
