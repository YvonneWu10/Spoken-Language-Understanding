...
def read_data_and_save(label_fn, save_fp, bin_norm=False, rm_null=False):
    label_data = json.load(open(label_fn, 'r', encoding='UTF-8'))
...
parser.add_argument('--data_dir', default='/home/huangyixin/NLP/data', help='raw data directory')
...
train_data_fn = os.path.join(processed_data_dir, 'train')
valid_data_fn = os.path.join(processed_data_dir, 'valid')
test_data_fn = os.path.join(processed_data_dir, 'test')
memory_fn = os.path.join(processed_data_dir, 'memory.pt')
pre_log_fn = os.path.join(processed_data_dir, 'log')
...
for mode in ['train', 'valid', 'test']:
    if mode == "train":
        label_fn = os.path.join(data_dir, "train.json")
    elif mode == "valid":
        label_fn = os.path.join(data_dir, "development.json")
    else:
        label_fn = os.path.join(data_dir, "test_unlabelled.json")
    word_list, label_set, sls, nps, nds, sys_mem = read_data_and_save(
        label_fn, fps[mode], bin_norm=opt.bin_norm, rm_null=opt.rm_null
    )
    if mode == 'train':
