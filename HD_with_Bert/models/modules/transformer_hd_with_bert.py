...
class TransHierarDecoder_withBERT(nn.Module):
    def __init__(self, dec_word_vocab_size, act_vocab_size, slot_vocab_size,
        ...
        self.dec_word_vocab_size = dec_word_vocab_size
    ...
    def forward(self, src_seq, enc_out, sent_level_fea, acts, act_slot_pairs,
            values, values_seg,
        ...
        '''
        # cases are that there are no semantics extracted   
        # if act_slot_pairs is None:
        #     return None, None, None
            
        # act prediction, a multi-label classification
        # ideally, if the actual semantics is empty, all scores of this act scores should be low
        ...
        value_scores, *_ = self.decoder(values, values_seg, src_seq, enc_out, ext_fea=ext_fea,
    ...
    def decode(self, src_seq, enc_out, sent_level_fea, extra_zeros, enc_batch_extend_vocab_idx, word_vocab, label_vocab, oov_list, device, tokenizer=None):
        ...
        act_pairs = [(j, label_vocab.idx2act[j]) for j in pred_acts]
        remain_acts = []
        ...
        act_inputs = bert_xlnet_inputs.prepare_inputs_for_bert_xlnet_act([act_input], word_vocab, label_vocab, tokenizer, device)
        ...
        for i in range(num_acts):
            ...
            slot_pairs = [(j, label_vocab.idx2slot[j]) for j in pred_slots]
        ...
        act_slot_pairs_batch = bert_xlnet_inputs.prepare_inputs_for_bert_xlnet_slot(
            [act_slot_pairs], word_vocab, label_vocab, tokenizer, device)
        ...
        values = self.decode_value_greedy(src_seq, enc_out, ext_fea,
            extra_zeros, enc_batch_extend_vocab_idx, oov_list,
    ...
    def decode_value_greedy(self, src_seq, enc_out, ext_fea,
            extra_zeros, enc_batch_extend_vocab_idx, oov_list,
            device=None, max_seq_len=10, tokenizer=None):
        ...
        for i in range(bs):
            ...
            for j in range(max_seq_len - 1):
                ...
                word_prob, *_ = self.decoder(dec_seq, dec_seg, src_seq, enc_out, ext_fea=ext_fea[i:i+1],
                    return_attns=True, extra_zeros=extra_zeros, extend_idx=enc_batch_extend_vocab_idx)
