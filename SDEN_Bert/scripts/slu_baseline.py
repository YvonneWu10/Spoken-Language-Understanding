...
train_dataset = Example.load_dataset(train_path, flag="asr")
# print([ex.utt for i in range(20) for ex in train_dataset[i]])
dev_dataset = Example.load_dataset(dev_path, flag="asr")
# print([ex.utt for i in range(20) for ex in dev_dataset[i]])
...
# check_point = torch.load(open('best_models/model2.bin', 'rb'), map_location=device)
...
def predict():
    ...
    test_dataset = Example.load_dataset(test_path, flag="asr")
    predictions = {}
    with torch.no_grad():
        for i in range(0, len(test_dataset), args.batch_size):
            ...
            for ci, conv in enumerate(current_batch.examples):
                model.clear_memory()
                for ui, ex in enumerate(conv):
                    pred = model.decode(Example.label_vocab, current_batch, ci, ui)
                    did = current_batch.did[ci][ui]
                    predictions[did] = []
                    for pi, p in enumerate(pred):
                        predictions[did].append(p)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            }, open('best_models/model12.bin', 'wb'))
