...
from model.slu_Transformer_parallel import TagTransformerParallel
from model.slu_Transformer_parallel2 import TagTransformerParallel2
...
if args.connection == 'Serial':
    model = TagTransformerParallel2(args).to(device)
else:
    model = TagTransformerParallel(args).to(device)

Example.word2vec.load_embeddings(model.word_embed, Example.word_vocab, device=device)

if args.testing:
    check_point = torch.load(open('./output/transformer_model.bin', 'rb'), map_location=device)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            }, open('./output/transformer_model.bin', 'wb'))
    ...
    if args.results_save:
        if not os.path.exists('results'):
            os.makedirs('results')
        filepath = file_path = os.path.join('results', filename)
        with open(filepath, 'a') as f:
            print('lr: %f, drop_out: %f, Drnn: %d, Arnn: %d, RNN: %s, Word: %s, tran: %s, connection: %s, cnn: %s' % (args.lr, args.dropout, args.num_layer_rnn, args.num_layer_attn, args.encoder_cell, args.word_embedding, args.trainset_spoken_language_select, args.connection, str(args.CNN)), file=f)
            print('FINAL BEST RESULT: \tEpoch: %d\tDev loss: %.4f\tDev acc: %.4f\tDev fscore(p/r/f): (%.4f/%.4f/%.4f)' % (best_result['iter'], best_result['dev_loss'], best_result['dev_acc'], best_result['dev_f1']['precision'], best_result['dev_f1']['recall'], best_result['dev_f1']['fscore']), file=f)
            print('\n', file=f)
else:
    start_time = time.time()
    metrics, dev_loss = decode('dev')
